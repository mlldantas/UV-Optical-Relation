...
x1 <- UV_dat$extinction          # random uniform variable 1
x2 <- UV_dat$dn4000_synth         # random uniform variable 2
y <- asinh((UV_dat$fuv_flux_obs-UV_dat$fuv_flux_syn)/UV_dat$fuv_flux_obs)     # create y as adjusted random normal variate


fit <- lm(y~x1+x2+I(x1^2)+I(x2^2))             # Compute the linear regression (z = ax + by + d)
